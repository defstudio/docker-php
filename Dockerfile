...
ARG PHP_NODE_VERSION=0
RUN if [ "${PHP_NODE_VERSION}" -ne "0" ] ; then \
    curl -sL "https://deb.nodesource.com/setup_$PHP_NODE_VERSION.x" | bash - && \
    apt-get update && \
...
RUN install-php-extensions imagick/imagick@65e27f2bc0 && \
    rm -rf /tmp/pear && \
